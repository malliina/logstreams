...
class DoobieDatabaseAuth(db: DoobieDatabase) extends UserService {
  def add(creds: BasicCredentials): Future[Either[AlreadyExists, Unit]] = db.run {
  ...
  def update(creds: BasicCredentials): Future[Either[DoesNotExist, Unit]] = db.run {
  ...
  def remove(user: Username): Future[Either[DoesNotExist, Unit]] = db.run {
  ...
  def all(): Future[Seq[Username]] = db.run {
