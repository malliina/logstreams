...
class SocketsBundle(
  ...
  val savedEvents
    : Source[AppLogEvents, NotUsed] = onlyOnce(logsSource.mapAsync(parallelism = 10) { ins =>
    db.insert(ins.events).map(written => AppLogEvents(written.rows.map(_.toEvent))).unsafeToFuture()
  })
        ...
        StreamsQuery(rh)
          .map { query =>
            ...
            val concatEvents: Source[AppLogEvents, NotUsed] =
              Source
                .future(db.events(query).unsafeToFuture())
